...
# (Optional) Year (e.g. 2024): Specifies the year when the job should run. This field is optional, and is typically not used unless the job needs to run on a specific year.
...
#this cron expression means "run the task every 12 hours on Saturdays at midnight". 
    # The "0" in the first field means "at the start of the hour", the "*/12" in the second field means
    # "every 12 hours", the "6" in the fifth field means "on Saturdays", and the "0" in the sixth
    # field means "at midnight". The third and fourth fields, " * *", represent "every
    # day and every month", respectively.
...
with DAG(
    ...
    schedule_interval = '0 */12 * * 6 0', # min, hour, day of month, month, day of the week, year
    catchup = True
) as dag:
